...
function getStatistics(){  
   ...
   include $_SERVER['DOCUMENT_ROOT']."/app/get_scholare_statistics.php";
            $sql0="select * from statisticsTable WHERE year='".date("Y")."'";
              ...
              $totalCite= $data["totalCite"] ?? 0;
              $hIndexScore=$data["hIndexScore"] ?? 0;
              $i10IndexScore=$data["i10IndexScore"] ?? 0;
               }
              }
...
$data["totalCite"]=$totalCite ?? 0;
$data["hIndexScore"]=$hIndexScore ?? 0;
$data["i10IndexScore"]=$i10IndexScore ?? 0;
         return $data;
