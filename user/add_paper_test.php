...
-->                                                
                      ...
                      <hr>              
                                    ...
                                    <h2>Current Author List   <?php $myid = session_id();echo $myid; ?> </h2>
       ...
       <thead>
            ...
            <!-- $_SESSION'deki yazarlar burada listelenecek -->
        </tbody>
...
?>
    ...
    <h2>Yazar Ekle</h2>
    <form id="authorForm">
        <input type="hidden" id="authorIndex">
        ...
        <label for="orcid">ORCID Number:</label><br>
        <input type="text" id="orcid" required><br><br>
        <label for="institution">Institution:</label><br>
        <input type="text" id="institution" required><br><br>
        <button type="submit" name ="kapat">Kaydet</button>
    </form>
...
if (!empty($authors)) {
    foreach ($authors as $index => $author) {
        echo "<p><strong>Yazar " . ($index + 1) . ":</strong></p>";
        echo "<ul>";
        echo "<li>First Name: " . htmlspecialchars($author['firstName']) . "</li>";
        echo "<li>Last Name: " . htmlspecialchars($author['lastName']) . "</li>";
        echo "<li>Email: " . htmlspecialchars($author['email']) . "</li>";
        echo "<li>ORCID: " . htmlspecialchars($author['orcid']) . "</li>";
        echo "<li>Institution: " . htmlspecialchars($author['institution']) . "</li>";
        echo "</ul>";
    }
} else {
    echo "<p>Henüz yazar kaydedilmedi.</p>";
}
?>
    ...
    <script>
        ...
        function generateUUID() {
            return 'bilgi-xxxxxx-xxxxxx-xxxxxx-xxxxxx'.replace(/[x]/g, function() {
        ...
        function saveAuthors(authors) {
            localStorage.setItem(storageKey, JSON.stringify(authors));
        }
        
        // Veri okuma
        function getAuthors() {
            let authors = localStorage.getItem(storageKey);
        ...
        function getAuthors_old() {
            let authors = localStorage.getItem('authors');
            return authors ? JSON.parse(authors) : [];
        }

        function saveAuthors_old(authors) {
        ...
        function loadAuthors() {
            ...
            authors.forEach((author, index) => {
                ...
                row.insertCell(0).innerText = author.firstName;
                row.insertCell(1).innerText = author.lastName;
                row.insertCell(2).innerText = author.email;
                row.insertCell(3).innerText = author.orcid;
                row.insertCell(4).innerText = author.institution;
                const actionsCell = row.insertCell(5);

                const editButton = document.createElement('button');
                editButton.innerText = 'Düzenle';
                editButton.onclick = () => editAuthor(index);
                actionsCell.appendChild(editButton);

                const deleteButton = document.createElement('button');
                deleteButton.innerText = 'Sil';
                deleteButton.onclick = () => deleteAuthor(index);
                actionsCell.appendChild(deleteButton);

                if (index > 0) {
                    const upButton = document.createElement('button');
                    upButton.innerText = 'Yukarı';
                    upButton.onclick = () => moveAuthor(index, -1);
                ...
                if (index < authors.length - 1) {
                    ...
                    downButton.onclick = () => moveAuthor(index, 1);
        ...
        function editAuthor(index) {
            ...
            document.getElementById('orcid').value = author.orcid;
            document.getElementById('institution').value = author.institution;
            document.getElementById('authorIndex').value = index;
        }
      ...
      function add_AuthorList() {
            const authors = getAuthors();
            document.getElementById('id01').style.display='none';
            const authorList2 = document.getElementById('authorList2');
            authorList2.innerHTML = '';

            authors.forEach((author, index) => {
                const row = authorList2.insertRow();
                row.insertCell(0).innerText = author.firstName;
                row.insertCell(1).innerText = author.lastName;
                row.insertCell(2).innerText = author.email;
                row.insertCell(3).innerText = author.orcid;
        ...
        function moveAuthor(index, direction) {
            const authors = getAuthors();
            const [movedAuthor] = authors.splice(index, 1);
            authors.splice(index + direction, 0, movedAuthor);
            saveAuthors(authors);
            loadAuthors();
        ...
        document.getElementById('authorForm').addEventListener('submit', function(e) {
            e.preventDefault();
            const authors = getAuthors();
            const index = document.getElementById('authorIndex').value;
            ...
            document.getElementById('authorIndex').value = '';
            loadAuthors();
        });

        // Sayfa yüklendiğinde yazarları listele
        loadAuthors();
    </script>
  
    
    <button onclick="add_AuthorList()" type="submit" name ="kapat" >KAPAT</button>
